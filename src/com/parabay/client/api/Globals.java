...
import com.parabay.client.gears.Factory;
...
public class Globals  {
	...
	public String getToken() {
		...
		if (token==null || token.length() == 0) {
			token = (String) Globals.getInstance().getDatabaseCacheModel().get("userToken");
		}
	...
	public IDataCollection getDataCollectionForDataQuery(String app, Map<String, Object> dataQuery) {
		
		String name = (String) SharedUtils.getValue(dataQuery,
				"name");
		String kind = (String) SharedUtils.getValue(
				dataQuery, "type_of");
		String orders = (String) SharedUtils.getValue(
				dataQuery, "orders");
		String filters = (String) SharedUtils.getValue(
				dataQuery, "filters");
		String params = (String) SharedUtils.getValue(
				dataQuery, "params");
		...
		if (null == dataCollection) {
			...
			if (null != orders) {
				...
				for(String s : arr) {
					dataCollection.getQuery().getOrders().add(s);
				}
			}
			
			if (null != filters && null != params) {
				String arrFilters[] = filters.split(",");
				String arrParams[] = params.split(",");
				
				if (arrFilters.length == arrParams.length) {
					for (int i = 0; i < arrFilters.length; i++) { 
						dataCollection.getQuery().addFilter(arrFilters[i], arrParams[i]);
					}
				}
			}
	...
	public Database getClientDatabase() {
		...
		if (null == this.clientDatabase) {
			...
			this.clientDatabase = f.createDatabase("DefaultDb");
		}
